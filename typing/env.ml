      ...
      List.iter (fun ((item : Subst.Lazy.signature_item), path) ->
        match item with
          SigL_value(id, decl, _) ->
            ...
            let vda =
              { vda_description = decl'; vda_address = addr; vda_shape; vda_mode = Alloc_heap }
            in
...
let add_value ?check ?shape ?(mode = Alloc_heap) id desc env =
...
let enter_value ?check name desc env =
  ...
  let addr = value_declaration_address env id desc in
  let env = store_value ?check Alloc_heap id addr desc (Shape.leaf desc.val_uid) env in
  (id, env)
